...
Každé 3h (přes GitHub Actions) kontroluje Blizzard stránky:
  ...
  Parsování textu → strukturovaná JS data zajišťuje Google Gemini API (zdarma).
"""
...
def parse_with_gemini(article_url: str, article_text: str) -> str:
    """Pošle text článku Gemini a vrátí JS object literal."""
...
def main() -> None:
    ...
    try:
        ...
        if not current_title:
            ...
            new_js = parse_with_gemini(latest["url"], body)
            print(f"  🤖 Gemini naparsoval objekt ({len(new_js)} znaků)")
    ...
    for url in new_urls:
        try:
            ...
            body         = get_article_body(article_html)
            new_js       = parse_with_gemini(url, body)
            print(f"  🤖 Gemini naparsoval objekt ({len(new_js)} znaků)")
